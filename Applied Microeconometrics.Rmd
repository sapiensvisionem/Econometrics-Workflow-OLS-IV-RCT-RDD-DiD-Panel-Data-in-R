...
Randomized Control Trial is the best one can get with data source for stuying causality.
...
Reason for randomization is that treatment and control groups will not differ systematically with respect to other characteristics that affect outcomess. However, there is still a pure chance that some units with better or worse outcomes happen to end up in the treatment or control group. As the size of treatment and control groups increases, the influence of pure chance on the results will fall, so we can mitigate the effect of pure chance by increasing sample size.

Instead of being randomly assigned, if units select themselves into treatment and control groups, then we have a self-selection problem. The units rather than the researchers selected the type of treatment. Then, it is possible that the difference in outcome indeed reflects the causal effect of treatment but an alternative explanation that cannot be ruled out is that the units with a systematic traits will have had resulted in different outcomes anyway. This can cause systematic difference in outcomes, rather than the difference of treatment.  Selection bias can exist if there is a correlation between error term and treatment status.

How can we measure the effect of treatment? One way is to take a difference in average outcomes between the two groups, which consists of two parts: average treament effect on the treated and selection bias. The selection problem arises because at a given point in time an individual can only be treated or not treated. We don't know the case where individual was observed without treatment and cannot observe the counterfactual.

## Using OLS

For random assignemnt to work we need the independence assumption between treatment and other control variables.  and there are no observed or unobserved differences between treated or untreated. 

We can use OLS on randomly assigned data. OLS estimate is called the difference estimator. 

## Randomization Check
There are informal tests to see whether is a problem with randomization:
1. Run a regression using data before the treatment started using the treatment status, and this should not have any effect. This is a way to test any difference in potential outcomes between those who get treated and those who don't: We run  regression using pre-treatment data and test balance in pre-treatment variable
2. Add control variables, and if the treatment is random, then the estimated coefficient does not change. If control variables are uncorrelated with the treatment, then they will not affect the estimated treatment effect (MLR and SLR coefficients will be similar)

Adding control variables has the benefit that it may generate more precise estimate of causal effect:
...
2. non-compliance: when units drop out of treatment, the difference in outcomes between the treatment and the control groups now estimates the average impact of offering the treatment, usually called intention-to-treat

If attrition is random and affects the treatment and control groups in the same way, the estimates would remain unbiased.

## Data
Data comes from the paper 'Remedying Education: Evidence from two Randomized Experiments in India" by Banerjee, Cole, Duflo, and Linden (2007)
It contains data for analyzing the effect of assigning a remedial teacher to schools at random in the indian city of Vadodara.
...
Variables in Y1 and Y2 are normalized relative to the control group of the same year.
```{r}
...
```{r}
lmod4 <- lm(post_mathnorm ~ bal + pre_mathnorm, data=df1)
summ(lmod4)
```


```{r}
lmod5 <- lm(post_verbnorm ~ bal + pre_verbnorm, data=df1)
summ(lmod5)
```
